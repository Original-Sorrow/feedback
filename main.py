...
async def start(message: types.Message):
	...
	if result[0] == 0:
		...
		await message.answer('Вы были добавленны в чс бота')
...
async def h(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			...
			await message.answer('Сообщение отправленно администрации.')
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 0:
					...
					connection.commit()
					await message.answer('Пользователь успешно добавлен в чс.', reply_markup=kb.adm)
					await state.finish()
					await bot.send_message(message.text, 'Вы были добавлены в чс.')
				else:
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 1:
					...
					await message.answer('Данный пользователь не был в чс.', reply_markup=kb.adm)
					await state.finish()
		else:
			await message.answer('Пишите цифрами...\nВведи ID')
